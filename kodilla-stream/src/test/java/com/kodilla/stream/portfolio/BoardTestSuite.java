...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<LocalDate> averageTaskTime = project.getTaskLists().stream()
        ...
        for (LocalDate localDate : averageTaskTime) {
            averageTime.add(ChronoUnit.DAYS.between(localDate, LocalDate.now()));
        }
        System.out.println(averageTime);

        double average = IntStream.range(0, averageTime.size())
                .map(i -> Math.toIntExact(averageTime.get(i)))
                .average().getAsDouble();
